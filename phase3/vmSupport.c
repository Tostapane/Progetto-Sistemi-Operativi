 ...
 * We keep these static so other modules can't touch them directly and ruin our
 * day.
 */
static swap_t swapPool[POOLSIZE];
static int
    fifo_ptr; /* For the highly advanced FIFO page replacement algorithm */
...
void initSwapStructs() {
  ...
  for (int i = 0; i < POOLSIZE; i++) {
    swapPool[i].sw_asid =
        -1; /* -1 means this frame is wonderfully unoccupied */
...
void Pager() {
  ...
  /* 3. If it's a TLB-Modification exception, you're supposed to treat it as a
   * Program Trap */
  if (excCode ==
      1) { /* 1 is usually Mod, double check your architecture docs */
  ...
  if (swapPool[frameIndex].sw_asid != -1) {
     ...
     * To do this atomically, you MUST disable interrupts via the STATUS
     * register, do the update, and re-enable them. c) Write the contents of
     * this frame back to the owner's backing store (flash device).
     */
  ...
  /* You'll need to figure out the exact index in sup_privatePgTbl for this
   * pointer */
