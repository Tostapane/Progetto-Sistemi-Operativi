...
/* Extern declarations for your exception handlers (assuming you name them logically in sysSupport.c and vmSupport.c) */
extern void Pager();
...
void test() {
    ...
    shellSup->sup_exceptContext[0].stackPtr = (memaddr)&(shellSup->sup_stackTLB[499]); /* Stacks grow down [cite: 340] */
    shellSup->sup_exceptContext[0].status = IEPON | IMON | TEBITON; /* Kernel mode, ints on [cite: 338] */

    /* Context 1: General Exception Handler [cite: 337] */
    shellSup->sup_exceptContext[1].pc = (memaddr)GeneralExceptionHandler;
    shellSup->sup_exceptContext[1].stackPtr = (memaddr)&(shellSup->sup_stackGen[499]);
    shellSup->sup_exceptContext[1].status = IEPON | IMON | TEBITON; /* Kernel mode, ints on [cite: 338] */

    /* Initialize the Page Table for the shell [cite: 331] */
    for (int i = 0; i < MAXPAGES - 1; i++) {
        /* VPN from 0x80000 to 0x8001E, ASID = 1 [cite: 80, 83] */
        shellSup->sup_privatePgTbl[i].pte_entryHI = ((0x80000 + i) << VPNSHIFT) | (1 << ASIDSHIFT);
        /* V=0, D=1, G=0  */
        shellSup->sup_privatePgTbl[i].pte_entryLO = DIRTYON; 
    }
    /* Stack page initialization [cite: 81] */
    shellSup->sup_privatePgTbl[MAXPAGES - 1].pte_entryHI = (0xBFFFF << VPNSHIFT) | (1 << ASIDSHIFT);
